...
public class PlayerMovement : MonoBehaviour
{
    ...
    public bool IsWalking
    {
        get
        {
            return grounded && !playerInput.isSprinting &&
    ...
    [SerializeField]
    private float crouchLimit = 4f;
    [SerializeField]
    private float sprintLimit = 16f;
    [SerializeField]
    private float airLimit = 25;
    ...
    void CheckSlope()
    {
        if (Physics.Raycast(transform.position, Vector3.down, out slopeHit,
                            playerHeight / 2 + checkOffset))
        {
            if (slopeHit.normal != Vector3.up)
            {
                isOnSlope = true;
                slopeMoveDir =
                    Vector3.ProjectOnPlane(movementDirection, slopeHit.normal);
            }
            else
            {
                isOnSlope = false;
    ...
    void Move()
    {
        movementDirection = playerInput._horizontalInput * orientation.right +
                            playerInput._verticalInput * orientation.forward;
        movementDirection = movementDirection.normalized;
        if (isOnSlope)

        {
            rb.AddForce(slopeMoveDir * Time.deltaTime * movementForce *
                            moveMultiplier,
                        ForceMode.Acceleration);
        }
        else
        {
            rb.AddForce(movementDirection * Time.deltaTime * movementForce *
                            moveMultiplier,
                        ForceMode.Acceleration);
        }
    }
    ...
    {
        if (!grounded)
            currentSpeedLimit = airLimit;
        // if currently not sliding but crouching
        else if (canSlide && crouching)
            currentSpeedLimit = crouchLimit;
        else
        ...
        if (horizontalSpeed.magnitude > currentSpeedLimit)
        {
            ...
            rb.velocity =
                new Vector3(horizontalSpeed.x, rb.velocity.y, horizontalSpeed.z);
        }
    }
    void Slide()
    {
        ...
        {
            if (rb.velocity.magnitude >= sprintLimit - 1 &&
    ...
    void CutJump()
    {
        ...
        {
            rb.velocity =
                new Vector3(rb.velocity.x, rb.velocity.y * 0.3f, rb.velocity.z);
            jumpCancelled = true;
        }
    }
    void CheckGrounded()
    {
        grounded = Physics.Raycast(transform.position, Vector3.down,
                                   playerHeight * 0.5f + checkOffset, groundLayer);
    }
